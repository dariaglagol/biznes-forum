...
$other = mysql_real_escape_string($_POST['user-info-other']);

$search = mysql_fetch_array(mysql_query("SELECT `id` FROM `lider__forumtomsk` WHERE `hash` != '' && `hash` = '$hash'"));
if($search){
	$id = (int)$search['id'];
	mysql_query("UPDATE `lider__forumtomsk` SET `experience` = '$experience',`learn` = '$learn',`other` = '$other' WHERE `id` = '$id'");
